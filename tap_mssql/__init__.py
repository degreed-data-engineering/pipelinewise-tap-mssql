...
def do_sync_full_table(mssql_conn, config, catalog_entry, state, columns):
    ...
    LOGGER.info("do_sync_full_table() complete. State: {}".format(state))
...
def do_sync_log_based_table(mssql_conn, config, catalog_entry, state, columns):
    ...
    LOGGER.info("=======In do_sync_log_based_table=========")
    key_properties = common.get_key_properties(catalog_entry)
    ...
    LOGGER.info("initial_full_table_complete? = {}".format(initial_full_table_complete))
    if not initial_full_table_complete:
        LOGGER.info("not initial_full_table_complete")
        ...
        LOGGER.info("State Written: state = {}".format(state))
    ...
    if initial_load:
        LOGGER.info("Doing Full Table Sync")
        ...
        LOGGER.info("Initial Load Full Sync Completed.")
        LOGGER.info("State Modified: State = {}".format(state))

    else:
        LOGGER.info("Initial Load was already complete.")
